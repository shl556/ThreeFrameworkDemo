...
public class PdfCalendar {
    ...
    public void createPdf(String filename, Locale locale, int year) throws IOException, DocumentException {
        ...
        for (int month = 0; month < 12; month++) {
            ...
            //通过canvas写入table时必须显示指定宽度，可以不用指定锁定宽度，因为itext会忽略百分比形式的宽度设定
            ...
            // complete the table
            table.completeRow();
            //前面两个参数是写入canvas的起始行和结束行，-1表示所有的行都会被写入。后面两个参数是表格左顶点相对于页面左下角的x,y位移。该方法的返回值是表格写入后最后一行在y平移
            //最后一个参数是PdfContentByte实例，可以是一个，也可以是一个由PdfContentByte实例组成的数组
            //任何一个表格在写入pdf的过程中有四个绘图层，从下到上一次是BASECANVAS，任何写入到该层的内容都会被放到表格底下
            //BACKGROUNDCANVAS绘制背景色的层，LINECANVAS绘制表格边框等线条的层，TEXTCANVAS表格文本内容的层
            //当传入一个PdfContentByte实例时会按照上述顺序叠加，最上面的内容会覆盖下面的内容
