...
public class MemoryTests {
    ...
    private void createPdfWithPdfPTable(Writer writer, String filename) throws IOException, DocumentException, SQLException {
        ...
        //正常情况下往pdf写入内容时会先将内容写入缓存中，等到全部内容写完以后才刷新缓存，将缓存中的内容写入到pdf文档中
        //可以利用LargeElement接口的setComplete(true)方法，该方法显示的告诉ITEXT内容已经填充完毕可以刷新
        //缓存了，该接口的另外两个方法isComplete()方法和flushContent()方法由itext内部调用，当调用iscomplete
        //方法返回true时，就会调用flushContent方法刷新缓存了。通过该方法可以控制缓存大小，避免内存溢出问题
         //另外该方法仅对document.add()方法有效
