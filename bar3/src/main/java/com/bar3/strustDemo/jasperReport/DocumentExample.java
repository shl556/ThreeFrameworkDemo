...
public class DocumentExample {
	private static String FILE_DIR = "d:\\sample\\itext\\doc\\";
...
//		Rectangle rect = new Rectangle(PageSize.A4.rotate());
		...
		// 文档属性,注意只能在Open方法调用之前添加摘要，调用open方法后这些摘要信息会被写入输出流中，无法修改
		//同样的用于设置该文档其他页水印、页眉、页角、页数和尺寸等必须在open方法前设置
		doc.addTitle("Title@sample");
		doc.addAuthor("Author@rensanning");
		doc.addSubject("Subject@iText sample");
		doc.addKeywords("Keywords@iText");
		doc.addCreator("Creator@iText");
		// 设置页边距，单位为磅，默认边距是36磅，半英寸。依次为左右上下
		//你修改了页面尺寸，仅仅影响到下一页，如果你修改了页边距，则影响到全部，故慎用
		doc.setMargins(50, 20, 100, 40);
		doc.open();
	...
	private void setDocPassword() throws Exception {
		...
		PdfWriter writer = PdfWriter.getInstance(doc, out);
		// 设置密码为："World"
		//第一个参数为用户密码，第二个参数为使用者密码，第三个参数是许可权限，第四个参数是密码类型
...
//		document.add(new Paragraph(Document.);
	...
	private void setWatermark() throws Exception {
		...
		//获取图片实例
		Image img = Image.getInstance("resources/watermark.jpg");
		//设置图片位置
		img.setAbsolutePosition(200, 400);
		//画图,getUnderContent()的参数是页码，即在指定页码添加图片水印
...
//		over.setTextMatrix(30, 30);
		...
		over.endText();
		// 背景图
		Image img2 = Image.getInstance("resources/test.jpg");
		img2.setAbsolutePosition(300, 300);
		//压缩图片至指定大小
		img2.scaleToFit(200f, 400f);
...
//		document.add(new Paragraph(Document.getVersion()));
	...
	private void splitPDF() throws Exception {
		...
		dd.newPage();
		//writer.getImportedPage(reader, 1)获取指定页面的实例
	...
	private void sortpage() throws Exception {
		...
		doc.newPage();
		doc.add(new Paragraph("5 page"));
		//order为页码组成的int数组，最后页面的顺序按照页码的顺序排序
		int[] order = {4,3,2,1};
